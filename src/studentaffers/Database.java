      ...
      try{
          System.out.println("Database class Accessed");
            connection=DriverManager.getConnection(host,user,password); // it always throw exception
                 ...
                 String sqlCommand=      "create table studentaffairs.studentsdata(name Text,nationalid text,email text,phonenumber text,level text,department text,course1 text,grade1 text,course2 text,grade2 text,course3 text,grade3 text, course4 text,grade4 text,course5 text,grade5 text,course6 text,grade6 text)" ;       
